...
char select_img[] =	"<form action=\"\" method=\"post\" enctype=\"multipart/form-data\">\r\n"
					...
					"<input type=\"file\" name=\"surtur\">\r\n"
					"<button type=\"submit\">Submit</button>\r\n"
					"</form>\r\n";
char text[] =		"<img src=\"/surtur.jpeg\" alt=\"No img\">\r\n";
...
void show_pack_msg(char *pack) {
	if(strncmp(buff, "GET ", 4) == 0 && strstr(buff, "GET /surtur.jpeg")) printf("Browser request Image\n");
...
int main(int argc, char **argv) {
	...
	while(1) {
		...
		if (!pid) {
			...
			if(strncmp(buff, "GET ", 4) == 0 && strstr(buff, "GET /surtur.jpeg")) {
				
				FILE *image = fopen("surtur.jpeg", "rb");
				if(image == NULL) {
					printf("fopen error\n");
					exit(1);
				}
